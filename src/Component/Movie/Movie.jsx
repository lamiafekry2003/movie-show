...
//   const url = `https://api.themoviedb.org/3/movie/${type}?api_key=${apiKey}&page=${page}`; // Corrected endpoint and query parameters
...
//         </div>
//         <div 
//         className="  flex flex-col  justify-center items-center mx-4 md:grid gap-2 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
//           {data?.length > 0 ? (
//             data.map((item) => (
//               <div className="" key={item.id}>
//                 <Link
//                   to={`/home/movie/${item.id}`} // Correctly set media_type and id
...
// const fetchMovie = async (type, page) => {
//   const apiKey = "4506a33c6fd8b3db74243b36650bd7fb";
//   const url = `https://api.themoviedb.org/3/movie/${type}?api_key=${apiKey}&page=${page}`;
//   const response = await axios.get(url);
//   return response.data.results;
// };
  ...
  return (
    <div className="container mx-auto px-2 my-20">
      ...
      <div className="relative">
        ...
        <div 
          className="grid gap-2 mx-2 grid-cols-2 justify-items-center sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6"
        >
          {data?.length > 0 ? (
            data.map((item, index) => (
              <motion.div
              ...
              >
                <Link
                  to={`/home/movie/${item.id}`}
                  className="w-full min-w-[160px] max-w-[160px] sm:min-w-[220px] sm:max-w-[220px] md:min-w-[230px] md:max-w-[230px] h-80 overflow-hidden block relative hover:scale-105 transition-all rounded-lg"
                >
                  {item?.poster_path ? (
                    <img
                      src={`https://image.tmdb.org/t/p/w500/${item?.poster_path}`}
