...
const fetchMovie = async (type, page) => {
  const apiKey = "4506a33c6fd8b3db74243b36650bd7fb";
  const url = `https://api.themoviedb.org/3/tv/${type}?api_key=${apiKey}&page=${page}`;
  const response = await axios.get(url);
  return response.data.results;
};
  ...
  return (
    <div className="container mx-auto px-2 my-20">
      <Helmet>
        ...
        <meta name="description" content="Explore popular, top-rated, on-the-air, and airing-today TV shows" />
        <link rel="icon" href={tvIcon} />
      </Helmet>
      <div className="relative">
        <div className="flex flex-col lg:flex-row justify-center items-center lg:justify-between">
              ...
              >
                ...
                >
                  ...
                  <div className="absolute bottom-0 h-16 backdrop-blur-3xl w-full bg-black/60 p-2">
                    ...
                    <p>{moment(item?.first_air_date || item?.release_date).format("MMMM Do YYYY")}</p>
                  </div>
                </Link>
              </motion.div>
            ))
